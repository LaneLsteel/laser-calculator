...
const CleaningMode = ({ goToHomeScreen }) => {
  ...
  useEffect(() => {
    ...
    let pulseEnergy = !isNaN(numMaxEnergy) ? numMaxEnergy : (numPower / (repetitionRate * 1000)) * 1000;
    ...
    // --- NEW Motion & Productivity Calculations ---
    ...
    if (headPerf && !isNaN(focalLength) && focalLength > 0 && !isNaN(hatchLength) && !isNaN(initialVelocity) && !isNaN(maxVelocity)) {
        ...
        calcs.maxProductivity = (calcs.maxAvgSpeed * 100 * newSpotSize) / 10;
        
        const robotSpeedCalc = (actualFill * numScanSpeed) / hatchLength;
        const productivityRate = (calcs.avgSpeed * actualFill) / 100;
  ...
  }, [laserInfo, setupParameters, scanInfo, calculatorMode, calculatedScanInfo.displayFill, calculatedScanInfo.displayPulseOverlap]);
  ...
  useEffect(() => {
    ...
    const collectAllData = useCallback(() => {
        return {
            ...
            calculatedSpotSize, powerDensity, pulseSpacing, fluence, productivity, motionCalculations
        };
    }, [laserInfo, setupParameters, scanInfo, partInfo, calculatedSpotSize, powerDensity, pulseSpacing, fluence, productivity, motionCalculations]);
        ...
        return (
            <div className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="p-2 bg-gray-100 rounded-lg">
                        <Tooltip text="min(Hatch Length / 2, 0.5 * (Scan Speed^2 - Initial Velocity^2) / Max Accel (Linear))"><p className="font-semibold text-gray-700">Accel Distance (Line):</p></Tooltip>
                        <p className="text-gray-900">{accelDistance.toFixed(4)} mm</p>
                    </div>
                    <div className="p-2 bg-gray-100 rounded-lg">
                        <Tooltip text="Hatch Length - 2 * Accel Distance"><p className="font-semibold text-gray-700">Coast Distance:</p></Tooltip>
                        <p className="text-gray-900">{coastDistance.toFixed(4)} mm</p>
                    </div>
                    <div className="p-2 bg-gray-100 rounded-lg">
                        <Tooltip text="Time to accelerate to max speed"><p className="font-semibold text-gray-700">Accel Time:</p></Tooltip>
                        <p className="text-gray-900">{accelTime.toFixed(6)} s</p>
                    </div>
                    <div className="p-2 bg-gray-100 rounded-lg">
                        <Tooltip text="Coast Distance / Scan Speed"><p className="font-semibold text-gray-700">Coast Time:</p></Tooltip>
                        <p className="text-gray-900">{coastTime.toFixed(6)} s</p>
                    </div>
                     <div className="p-2 bg-gray-100 rounded-lg">
                        <Tooltip text="0.5 * Initial Velocity^2 / Max Accel (Linear)"><p className="font-semibold text-gray-700">Turn Around Distance:</p></Tooltip>
                        <p className="text-gray-900">{turnAroundDistance.toFixed(4)} mm</p>
                    </div>
                    <div className="p-2 bg-gray-100 rounded-lg">
                        <Tooltip text="Initial Velocity + Accel Time * Max Accel (Linear)"><p className="font-semibold text-gray-700">Achieved Max Speed:</p></Tooltip>
                        <p className="text-gray-900">{maxSpeed.toFixed(2)} mm/s</p>
                    </div>
                    <div className="p-2 bg-gray-100 rounded-lg">
                        <Tooltip text="2 * Accel Time + Coast Time + 0.0003"><p className="font-semibold text-gray-700">Line Time:</p></Tooltip>
                        <p className="text-gray-900">{lineTime.toFixed(6)} s</p>
                    </div>
                     <div className="p-2 bg-gray-100 rounded-lg">
                        <Tooltip text="2 * Initial Velocity / Max Accel (Linear) + 2 * Tracking Delay"><p className="font-semibold text-gray-700">Turn Around Time:</p></Tooltip>
                        <p className="text-gray-900">{turnAroundTime.toFixed(6)} s</p>
                    </div>
                     <div className="p-2 bg-gray-100 rounded-lg">
                        <Tooltip text="Turn Around Time + Line Time"><p className="font-semibold text-gray-700">Total Line Time:</p></Tooltip>
                        <p className="text-gray-900">{totalLineTime.toFixed(6)} s</p>
                    </div>
                </div>
                 <div className="mt-4 space-y-3">
                    ...
                    <div className="p-3 bg-indigo-50 rounded-lg text-center">
                         <div className="flex justify-center items-center">
                            <Tooltip text="(Fill * Scan Speed) / X Direction"><h3 className="text-md font-semibold text-indigo-700 mr-2">Robot Speed:</h3></Tooltip>
                    ...
                    <div className="p-3 bg-teal-50 rounded-lg text-center">
                        ...
                        <p className="text-lg font-bold text-teal-800">{convertAndDisplay(rate, productivityUnit, 'areaRate_mm2_per_s', 4)} {productivityUnit.replace('_per_', '/').replace('2', '²')}</p>
                    </div>
                </div>
            </div>
        );
